...
Additionally, also enable the native "Email/Password" sign in method (without enabling "email link for passwordless sign in"). This is used for device ID login.
...
Next, you need an API key for google maps. You'll need to enable billing for this. Follow the [official steps](https://developers.google.com/maps/documentation/android-sdk/get-api-key?setupProd=configure) to get one (make sure you choose your firebase project). Once you have the key, put in `MAPS_API_KEY=<PASTE THE API KEY>` in its own line inside `keys.properties`. DO NOT surround this API key in quotes.
