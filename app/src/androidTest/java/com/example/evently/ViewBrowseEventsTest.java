...
public class ViewBrowseEventsTest extends EmulatedFragmentTest<BrowseEventsFragment>{
	...
	public void testViewingEvents() throws InterruptedException {
		...
		// Test if each event added shows up on the recyclerview, one of the events has a closed waitlist
		for (final var expectedEvent : mockEvents) {
			...
			else
			{
				assertRecyclerViewItem(
						R.id.event_list,
						p(R.id.content, expectedEvent.name()),
						p(R.id.txtselection_date, MessageFormat.format(
						"Selection on {0}",
								some_date.format(expectedEvent.selectionTime().toInstant()))),
						p(R.id.txtDate, some_date.format(expectedEvent.eventTime().toInstant())));
			}
		}
